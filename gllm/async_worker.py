...
class AsyncWorker:
    ...
    def init(self):
        ...
        if self.pp_rank == 0:
            ...
            self.schedule_socket = make_socket(
                zmq_ctx, self.schedule_ipc_path, zmq.PULL)
            # rank 0 => main process
            self.output_socket = make_socket(
                zmq_ctx, self.output_ipc_path, zmq.PUSH)
            # seqs to schedule
            ...
            for i in range(1, self.pp_size):
                self.gpu_schedule_socket.append(make_socket(
                    zmq_ctx, f'{self.schedule_ipc_path}_{i}', zmq.PUSH))
            if self.pp_size != 1:
                # last rank => rank 0 : next tokens
                self.token_socket = make_socket(
                    zmq_ctx, self.token_ipc_path, zmq.PULL)
        else:
            # rank 0 => other ranks : batched seqs
            self.gpu_schedule_socket = make_socket(
                zmq_ctx, f'{self.schedule_ipc_path}_{self.pp_rank}', zmq.PULL)
        ...
        if self.pp_rank == self.pp_size - 1 and self.pp_size != 1:
            # last rank => rank 0 : next tokens
            self.token_socket = make_socket(
                zmq_ctx, self.token_ipc_path, zmq.PUSH)
    ...
    def schedule(self):
        ...
        if self.pp_size > 1 and prefill_token_budget != 0:
            ...
            prefill_token_budget = min(
                self.maxp, prefill_token_budget)
        prefill_batched_token_nums = 0
        ...
        if len(self.seqs_to_decode) + len(self.seqs_to_prefill) != 0 and len(self.batch_running) < self.pp_size:
            ...
            if len(schedule_seqs) != 0:
                ...
                if self.pp_size > 1:
                    ...
                    for i in range(1, self.pp_size):
                        self.gpu_schedule_socket[i -
                                                 1].send(seqs_bytes, copy=False)
