...
class InputData():
    def __init__(self, use_buffer:bool, memory_manager: MemoryManager, 
                 max_seq_length, max_running_seqs=None):
        ...
        if use_buffer:
            ...
            self.slot_mapping = torch.zeros(max_seq_length * max_running_seqs, dtype=torch.int64)
            self.block_table = torch.zeros((max_running_seqs, self.max_num_block), dtype=torch.int32)
    ...
    def copy_to_input_buffer(self):
        ...
        self.tokens[:self.tokens_cpu.shape[0]].copy_(self.tokens_cpu, non_blocking=True)
        if hasattr(self, "mrope_positions_cpu") and self.mrope_positions_cpu is not None:
            ...
            self.positions[:self.positions_cpu.shape[0]].copy_(self.positions_cpu, non_blocking=True)
        self.slot_mapping[:self.slot_mapping_cpu.shape[0]].copy_(self.slot_mapping_cpu, non_blocking=True)
        self.block_table[:self.block_table_cpu.shape[0]].copy_(self.block_table_cpu, non_blocking=True)
        self.seq_lens[:self.seq_lens_cpu.shape[0]].copy_(self.seq_lens_cpu, non_blocking=True)
        self.query_start_loc[:self.query_start_loc_cpu.shape[0]].copy_(self.query_start_loc_cpu, non_blocking=True)
    ...
    def set_input_from_prebuilt(self, input_data):
        ...
        mla_attrs_copy = ["num_actual_tokens", "num_decodes", "num_decode_tokens", "num_prefills",
                          "max_context_len", "decode_seq_lens_cpu", "prefill_max_query_len", 
                          "prefill_query_start_loc_cpu", "chunk_starts_cpu", "chunk_seq_lens_cpu", 
                          "cu_seq_lens_cpu"]
    ...
    def _cal_seq_lens(self, seqs):
        ...
        return max(seq_lens), torch.tensor(seq_lens, dtype=torch.int32, device="cpu", pin_memory=True)
    ...
    def _cal_query_start_loc(self, seqs):
        ...
        return max(query_lens), torch.from_numpy(np.cumsum(query_lens)).to(device='cpu', dtype=torch.int32)
    ...
    def _cal_mla_metadata(self, seqs:List[Sequence]):
        ...
        if self.num_prefills > 0:
            ...
            self.prefill_max_query_len, self.prefill_query_start_loc_cpu = self._cal_query_start_loc(prefill_seqs)
            ...
            if self.max_context_len > 0:
                ...
                self.chunk_starts_cpu = torch.arange(num_chunks, dtype=torch.int32, device="cpu", pin_memory=True) \
                                .unsqueeze(1).expand(-1, self.num_prefills) \
                                * max_context_chunk
                chunk_ends = torch.min(context_lens.unsqueeze(0),
                                    self.chunk_starts_cpu + max_context_chunk)
                self.chunk_seq_lens_cpu = (chunk_ends - self.chunk_starts_cpu).clamp(min=0)
                ...
                self.cu_seq_lens_cpu = torch.zeros(
                    ...
                    device="cpu",
                    pin_memory=True)
                torch.cumsum(self.chunk_seq_lens_cpu,
                                dim=1,
    ...
    def _set_mla_metadata(self):
        if self.num_prefills > 0:
            self.prefill_query_start_loc[:self.prefill_query_start_loc_cpu.shape[0]].copy_(self.prefill_query_start_loc_cpu, non_blocking=True)
        if self.num_decodes > 0:
            self.decode_seq_lens[:self.decode_seq_lens_cpu.shape[0]].copy_(self.decode_seq_lens_cpu, non_blocking=True)
        ...
        ) if self.num_decodes > 0 else None
        ...
        ) if self.num_prefills > 0 and self.max_context_len > 0 else None
        
        prefill_metadata = MLACommonPrefillMetadata(
            block_table=self.get_block_table()[self.num_decode_tokens:],
            query_start_loc=self.prefill_query_start_loc[:self.prefill_query_start_loc_cpu.shape[0]],
