...
def send_pp_data(input_data:InputData, output, dst):
    ...
    send_tensor(input_data.slot_mapping_tensor)
    send_tensor(input_data.positions)
    ...
    if not input_data.computed_prompt:
        send_tensor(input_data.seq_start_loc)
        if input_data.prefix_prefill:
            obj_list = [input_data.max_seq_len, input_data.max_query_len]
            dist.send_object_list(obj_list, dst)
            send_tensor(input_data.block_table)
            send_tensor(input_data.query_start_loc)
        else:
            obj_list = [input_data.max_seq_len]
            dist.send_object_list(obj_list, dst)
    else:
        send_tensor(input_data.cache_seqs_len)
        send_tensor(input_data.block_table)
    
    # send hidden_states, residual
    send_tensor(output[0])
    send_tensor(output[1])
    
def recv_pp_data(dtype, memory_manager, src):
    slot_mapping_tensor = recv_tensor(dtype, src)
    positions = recv_tensor(dtype, src)
    ...
    if not computed_prompt:
        seq_start_loc = recv_tensor(dtype, src)
        if prefix_prefill:
            obj_list = [None, None]
            dist.recv_object_list(obj_list,src)
            max_seq_len,max_query_len = obj_list
            block_table = recv_tensor(dtype, src)
            query_start_loc = recv_tensor(dtype, src)
        ...
        cache_seqs_len = recv_tensor(dtype, src)
        block_table = recv_tensor(dtype, src)
    ...
    hidden_states = recv_tensor(dtype, dist.get_rank()-1)
    residual = recv_tensor(dtype, dist.get_rank()-1)
