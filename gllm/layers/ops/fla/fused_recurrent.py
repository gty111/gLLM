...
from gllm.layers.ops.fla.op import exp
...
# the committed column via ``num_accepted``. No copy / rotation between steps.
# ---------------------------------------------------------------------------
...
def fused_recurrent_gdn_spec_fwd_kernel(
    ...
    if USE_INITIAL_STATE:
        if IS_CONTINUOUS_BATCHING:
            if IS_SPEC_DECODING:
                ...
                i_t0 = 0
            state_idx = tl.load(
                ssm_state_indices + i_n * stride_indices_seq + i_t0 * stride_indices_tok
            ).to(tl.int64)
            if state_idx < 0:
                return
            p_h0 = h0 + state_idx * stride_init_state_token
    ...
    for i_t in range(0, T):
        ...
        # Write this token's post-state into column i_t of the seq's block table.
        if IS_CONTINUOUS_BATCHING:
            dst_idx = tl.load(
...
def fused_recurrent_gdn_spec(
    ...
    Returns the per-token output ``o`` (``[B, T, HV, V]``); state is updated in
    place in ``state_source``.
    """
    B, T, H, K, V = *k.shape, v.shape[-1]
    HV = v.shape[2]
    N = B if cu_seqlens is None else len(cu_seqlens) - 1
    ...
    stride_state_token = state_source.stride(0)
    if ssm_state_indices.ndim == 1:
    ...
    fused_recurrent_gdn_spec_fwd_kernel[grid](
        ...
        BK=BK,
        BV=BV,
        stride_init_state_token=stride_state_token,
