...
class MemoryManager():
    def __init__(self, layer_id, page_num_segment=1024, token_num_page=16, kv_head_num=8, kv_head_dim=128):
        '''
        ...
        # print(num_max_pages)
        self.segments = [
            Segment(layer_id, page_num_segment, token_num_page, kv_head_num, kv_head_dim, torch.bfloat16)]

    def store(self, k_cache: torch.Tensor, v_cache: torch.Tensor, seqs: List[Sequence]):
        cu_seqs_len = 0
        for seq in seqs:
            # prompt KV cache
            if len(seq.page_table[self.layer_id]) == 0:
                for i in range(0, seq.prompt_len, self.token_num_page):
                    page_num = self.segments[seq.segment_id].allocate()
                    seq.page_table[self.layer_id].append(page_num)
                    idx_right = min(seq.prompt_len, i+self.token_num_page)
                    self.segments[seq.segment_id].k_cache[page_num][0:idx_right-i].copy_(
                        k_cache[i+cu_seqs_len:idx_right+cu_seqs_len])
                    self.segments[seq.segment_id].v_cache[page_num][0:idx_right-i].copy_(
            ...
            # decode KV cache
                ...
                if (len(seq.token_ids)-1) % self.token_num_page == 0:
                    page_num = self.segments[seq.segment_id].allocate()
                    seq.page_table[self.layer_id].append(page_num)
                    self.segments[seq.segment_id].k_cache[page_num][0].copy_(
                        k_cache[cu_seqs_len])
                    self.segments[seq.segment_id].v_cache[page_num][0].copy_(
                        v_cache[cu_seqs_len])
                else:
                    offset = len(seq.token_ids) % self.token_num_page - 1
                    page_num = seq.page_table[self.layer_id][-1]
                    self.segments[seq.segment_id].k_cache[page_num][offset].copy_(
                        k_cache[cu_seqs_len])
                    self.segments[seq.segment_id].v_cache[page_num][offset].copy_(
                        v_cache[cu_seqs_len])
                cu_seqs_len += 1

    def free(self, seq: Sequence):
        for page_num in seq.page_table[self.layer_id]:
...
class Segment():
    def __init__(self,
        ...
        self.k_cache = torch.ones(
            (page_num_segment, token_num_page, kv_head_num, kv_head_dim), dtype=dtype, device='cuda')
